...
public class BinarySearch {
  ...
  public static void main(String[] args) {
    System.out.println("=".repeat(30) + " searchStrings " + "=".repeat(30));
    searchStrings();

    System.out.println("=".repeat(30) + " searchCats " + "=".repeat(30));
    searchCats();
  }

  public static void searchStrings() {
    var names = Arrays.asList("John", "Martin", "Paula", "Ann");
  ...
  public static void searchCats() {
    var fluffy = new Cat("Fluffy", 1);
    var bella = new Cat("Bella", 5);
    var cats = Arrays.asList(fluffy, bella);

    Collections.sort(cats);      // must sort first by natural order - name ascending
    System.out.println(cats);    // [Cat{name=Bella, age=5}, Cat{name=Fluffy, age=1}]
    ...
    //      will be found.
    System.out.println("Bella is at index: " + Collections.binarySearch(cats, bella)); // 0

    // set up the Comparator<T>
    //    int compare(T o1, T o2)
    var byAge = Comparator.comparingInt(Cat::getAge);
    Collections.sort(cats, byAge); // sort by age ascending
    System.out.println(cats);      // [Cat{name=Fluffy, age=1}, Cat{name=Bella, age=5}]
    // API: Searches the specified list for the specified object using the binary search algorithm.
    ...
    System.out.println("Bella is at index: " + Collections.binarySearch(cats, bella, byAge)); // 1
  }
}
